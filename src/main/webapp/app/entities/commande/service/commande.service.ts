...
export class CommandeService {
  ...
  protected convertDateFromClient(commande: ICommande): ICommande {
    return Object.assign({}, commande, {
      dateAchat: commande.dateAchat?.isValid() ? commande.dateAchat.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateAchat = res.body.dateAchat ? dayjs(res.body.dateAchat) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((commande: ICommande) => {
        commande.dateAchat = commande.dateAchat ? dayjs(commande.dateAchat) : undefined;
      });
    }
    return res;
  }
