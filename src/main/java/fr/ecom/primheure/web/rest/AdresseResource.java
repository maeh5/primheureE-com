...
/**
 * REST controller for managing {@link fr.ecom.primheure.domain.Adresse}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class AdresseResource {
    ...
    public ResponseEntity<Adresse> updateAdresse(@PathVariable(value = "id", required = false) final Long id, @RequestBody Adresse adresse)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, adresse.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Adresse> partialUpdateAdresse(
        ...
        Optional<Adresse> result = adresseRepository
            ...
            .map(existingAdresse -> {
                if (adresse.getAdresse() != null) {
                    existingAdresse.setAdresse(adresse.getAdresse());
                }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, adresse.getId().toString())
        );
    }
